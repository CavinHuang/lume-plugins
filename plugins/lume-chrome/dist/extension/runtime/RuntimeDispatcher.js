        ...
        try {
            ...
            switch (req.method) {
                ...
                case "browser_name_session":
                    await this.sessions.name(ctx, p.name);
                    await this.groups.name(ctx, p.name);
                    return ok(req.id, undefined);
                ...
                case "browser_user_claim_tab": {
                    ...
                    const lease = await this.leases.claimExisting(chromeTabId, ctx);
                    await this.groups.ensureGroup(ctx, chromeTabId, "Lume");
                ...
                case "create_tab": {
                    ...
                    if (p.options?.grouped !== false)
                        await this.groups.ensureGroup(ctx, tab.id, "Lume");
                    return ok(req.id, { tabId: lease.tabId });
                }
                case "get_tab":
                    await this.leases.get(p.tabId, ctx);
                    return ok(req.id, { tabId: p.tabId });
                case "selected_tab": {
                    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
                    if (!tab?.id)
                        return ok(req.id, {});
                    const lease = await this.leases.claimExisting(tab.id, ctx);
                    return ok(req.id, { tabId: lease.tabId });
                }
                case "list_tabs":
                case "get_session_tabs": return ok(req.id, await this.leases.listSessionTabs(ctx));
                case "release_tabs":
                    await this.leases.release(p.tabIds ?? [], ctx);
                    return ok(req.id, undefined);
                case "handoff_tabs":
                    await this.leases.handoff(p.tabIds ?? [], ctx);
                    return ok(req.id, undefined);
                case "resume_handoff_tabs": return ok(req.id, (await this.leases.resumeHandoff(ctx)).map(t => ({ tabId: t.tabId })));
                case "finalize_tabs":
                    await this.leases.finalize(ctx, p.keep ?? []);
                    await this.groups.cleanup(ctx);
                    return ok(req.id, undefined);
                case "close_tab": {
                ...
                case "tab_id": {
                    const l = await this.leases.get(p.tabId, ctx);
                    const tab = await chrome.tabs.get(l.chromeTabId);
                    return ok(req.id, { tabId: l.tabId, chromeTabId: l.chromeTabId, url: tab.url, title: tab.title });
                }
                ...
                case "navigate_tab_url": {
                    ...
                    if (p.options?.waitUntil)
                        await this.pw.waitForLoadState(id, p.options.waitUntil, p.options.timeoutMs ?? 15_000);
                    return ok(req.id, undefined);
                }
                case "navigate_tab_back":
                    await this.cdp.navigateHistory(await this.chromeTab(p.tabId, ctx), -1);
                    return ok(req.id, undefined);
                ...
                case "browser_viewport_reset":
                    for (const t of await this.leases.listSessionTabs(ctx))
                        await this.cdp.resetViewport(t.chromeTabId);
                    return ok(req.id, undefined);
                case "cua_click": {
                    const id = await this.chromeTab(p.tabId, ctx);
                ...
                case "cua_type":
                    ...
                    return ok(req.id, undefined);
                case "cua_keypress":
                    await this.cdp.keypress(await this.chromeTab(p.tabId, ctx), p.key);
                    return ok(req.id, undefined);
                case "dom_cua_get_visible_dom": return ok(req.id, await this.dom.visibleDom(await this.chromeTab(p.tabId, ctx)));
                case "dom_cua_click":
                    await this.dom.click(await this.chromeTab(p.tabId, ctx), p.node_id, false);
                    return ok(req.id, undefined);
                case "dom_cua_double_click":
                    await this.dom.click(await this.chromeTab(p.tabId, ctx), p.node_id, true);
                    return ok(req.id, undefined);
                case "dom_cua_type":
                    await this.dom.type(await this.chromeTab(p.tabId, ctx), p.node_id, p.text);
                    return ok(req.id, undefined);
                case "dom_cua_keypress":
                    await this.dom.keypress(await this.chromeTab(p.tabId, ctx), p.node_id, p.key);
                    return ok(req.id, undefined);
                case "dom_cua_scroll":
                ...
                case "playwright_wait_for_load_state":
                    await this.pw.waitForLoadState(await this.chromeTab(p.tabId, ctx), p.state ?? "load", p.timeoutMs ?? 10_000);
                    return ok(req.id, undefined);
                case "playwright_wait_for_timeout":
                    await new Promise(r => setTimeout(r, Math.min(Number(p.timeoutMs ?? 0), 30_000)));
                    return ok(req.id, undefined);
            ...
            if (error instanceof BrowserRuntimeException)
                return fail(req.id, error.code, error.message, error.details);
