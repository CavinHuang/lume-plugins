...
export class RuntimeDispatcher {
  ...
  private extensionCaps():BrowserBackendDescriptor{return{id:"extension",name:"Lume Chrome",type:"extension",protocolVersion:PROTOCOL_VERSION,generation:this.native.connectionGeneration(),metadata:{},capabilities:{browser:[{id:"visibility",description:"Show or hide the browser window."},{id:"viewport",description:"Set or reset the browser viewport."}],tab:[{id:"pageAssets",description:"Inventory and bundle rendered page assets."}]},apiSupportOverrides:{"BrowserUser.history":false,"Tabs.content":false,"Tab.clipboard":false,"Tab.content":false,"Tab.cua":false,"Tab.dev":false,"Tab.dom_cua":false,"Tab.getJsDialog":false,"PlaywrightAPI.elementInfo":false,"PlaywrightAPI.elementScreenshot":false,"PlaywrightAPI.frameLocator":false,"PlaywrightAPI.waitForEvent":false,"PlaywrightDownload.path":false,"PlaywrightFileChooser.setFiles":false,"PlaywrightFrameLocator.frameLocator":false,"PlaywrightLocator.and":false,"PlaywrightLocator.downloadMedia":false,"PlaywrightLocator.or":false,"PlaywrightLocator.type":false}};}
  async dispatch(req:RpcRequest):Promise<RpcResponse>{
    try{
      ...
      switch(req.method){
        ...
        case"cua_click":{const id=await this.chromeTab(p.tabId,ctx!);await injectScript(id,"dist/extension/content/overlay.js").catch(()=>undefined);await chrome.tabs.sendMessage(id,{type:"LUME_CURSOR_MOVE",x:p.x,y:p.y}).catch(()=>undefined);await this.cdp.click(id,p.x,p.y);return ok(req.id,undefined);}
        case"cua_double_click":await this.cdp.click(await this.chromeTab(p.tabId,ctx!),p.x,p.y,2);return ok(req.id,undefined);
        case"cua_move":await this.cdp.dispatchMouse(await this.chromeTab(p.tabId,ctx!),"mouseMoved",p.x,p.y);return ok(req.id,undefined);
        case"cua_drag":await this.cdp.drag(await this.chromeTab(p.tabId,ctx!),p.path??[]);return ok(req.id,undefined);
        case"cua_scroll":await this.cdp.dispatchMouse(await this.chromeTab(p.tabId,ctx!),"mouseWheel",p.x,p.y,{deltaX:p.scrollX??0,deltaY:p.scrollY??0});return ok(req.id,undefined);
