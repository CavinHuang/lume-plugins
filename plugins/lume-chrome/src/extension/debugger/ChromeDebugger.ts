...
export class ChromeDebugger {
  ...
  async sendRaw(tabId:number,method:string,params:Record<string,unknown>={},options:{target?:{sessionId?:string;targetId?:string};timeoutMs?:number}={}):Promise<unknown>{await this.ensureAttached(tabId);if(options.target?.sessionId&&options.target?.targetId)throw new Error("cdp target must specify only one of sessionId or targetId");const target=options.target?.targetId?{targetId:options.target.targetId}:options.target?.sessionId?{tabId,sessionId:options.target.sessionId}:{tabId};const command=chrome.debugger.sendCommand(target,method,params);if(!options.timeoutMs)return command;return await Promise.race([command,new Promise((_,reject)=>setTimeout(()=>reject(new Error(`Timed out waiting for CDP command: ${method}`)),options.timeoutMs))]);}
  ...
  logs(tabId:number,options:{levels?:string[];methods?:string[];limit?:number;filter?:string}={}){let logs=[...(this.devLogs.get(tabId)??[])];const methods=options.methods?new Set(options.methods):undefined;const levels=options.levels?new Set(options.levels.map(String)):undefined;if(methods)logs=logs.filter((entry:any)=>methods.has(String(entry.method)));if(levels)logs=logs.filter((entry:any)=>levels.has(String(entry.params?.level??entry.level??"")));if(options.filter){const needle=options.filter.toLowerCase();logs=logs.filter((entry)=>JSON.stringify(entry).toLowerCase().includes(needle));}const rawLimit=Number(options.limit??logs.length);const limit=Math.max(1,Math.min(Number.isFinite(rawLimit)?rawLimit:1,1000));return logs.slice(-limit);}
